...
// Export Higher Order Sortable Container Component with Varying Width Support for Grid
export default function sortableContainerVariedWidth(WrappedComponent, config = {withRef: false}) {
  return class extends Component {
    ...
    static defaultProps = {
      ...
      shouldCancelOver: function(prevIndex, newIndex) {
        // Cancel moving over when true
        return false;
      },
      leftCon: function(index) {
        // The most left node connected to index
        return index;
      },
      rightCon: function(index) {
        // The most right node connected to index
        return index;
      },
    ...
    static propTypes = {
      ...
      shouldCancelOver: PropTypes.func,
      leftCon: PropTypes.func,
      rightCon: PropTypes.func,
    ...
    handleStart = event => {
      const {distance, shouldCancelStart, beforeStart} = this.props;
    ...
    animateNodes() {
      const {transitionDuration, hideSortableGhost, onSortOver, shouldCancelOver, leftCon, rightCon} = this.props;
        ...
        if (this.axis.x) {
          if (this.axis.y) {
            ...
            if (
              ...
              if (nnewIndex === null && leftCon(index) == index) {
                nnewIndex = index;
              }
      ...
      for (let i = 0, len = nodes.length; i < len; i++) {
        ...
        if (this.axis.x) {
          if (this.axis.y) {
            // Calculations for a grid setup
            if (
              index < this.index && nnewIndex <= index &&
              (
                ((sortingOffset.left + windowScrollDelta.left) - offset.width <= edgeOffset.left &&
                (sortingOffset.top + windowScrollDelta.top) <= edgeOffset.top + offset.height) ||
                (sortingOffset.top + windowScrollDelta.top) + offset.height <= edgeOffset.top
              ...
              if(i == 0) { // If it is the first node
                ...
                let totalWd = prevNode.edgeOffset.left + anims[i - 1].x + prevNode.node.offsetWidth;
                if(nnewIndex == index) { // If it is the first node after the node being dragged
                  totalWd += this.width;
                }
                translate.x = totalWd - edgeOffset.left;
                translate.y = (prevNode.edgeOffset.top + anims[i - 1].y) - edgeOffset.top;
                if((totalWd + width) > this.containerBoundingRect.width) { // Doesn't fit, move to bottom row
                  translate.y = prevNode.edgeOffset.top + anims[i - 1].y + prevNode.node.offsetHeight - edgeOffset.top;
              ...
              let prevWidth = prevNode.node.offsetWidth;
              let realpNode = prevNode;
              let realAnim = anims[i - 1];
              if(index == (this.index + 1)) { // If it is the first node after the node being dragged
                prevWidth = 0;
                if(i > 1) {
                  realpNode = nodes[i - 2];
                  realAnim = anims[i - 2];
                  prevWidth = realpNode.node.offsetWidth;
                }
              }
              
              let totalWd = realpNode.edgeOffset.left + realAnim.x + prevWidth;
              translate.x = totalWd - edgeOffset.left;
              translate.y = (realpNode.edgeOffset.top + realAnim.y) - edgeOffset.top;
              if((totalWd + width) > this.containerBoundingRect.width) { // Doesn't fit, move to bottom row
                translate.x = nodes[0].edgeOffset.left - edgeOffset.left;
                translate.y = realpNode.edgeOffset.top + realAnim.y + realpNode.node.offsetHeight - edgeOffset.top;
              }
              ...
              let totalWd = realpNode.edgeOffset.left + realAnim.x + prevWidth;
              if(nnewIndex == prevNode.node.sortableInfo.index) { // If it is the first node after the new position
                totalWd += this.width;
              }
              translate.x = totalWd - edgeOffset.left;
              translate.y = (realpNode.edgeOffset.top + realAnim.y) - edgeOffset.top;
      ...
      this.newIndex = nnewIndex;
        if (this.newIndex == null) {
          this.newIndex = this.index;
        }
      if(shouldCancelOver(prevIndex, this.newIndex)) {
        this.newIndex = prevIndex;
        return;
      }
      
      for (let i = 0, len = nodes.length; i < len; i++) {
        nodes[i].node.style[`${vendorPrefix}Transform`] = `translate3d(${anims[i].x}px,${anims[i].y}px,0)`;
      }
